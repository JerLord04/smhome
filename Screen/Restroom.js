...
function Restroom({ navigation }, props) {
    const [value, onChangeText] = useState('');
    const [changeNameModal, setChangeNameModal] = useState(false);
    const [roomName, setRoomName] = useState('Rest Room');
    ...
    socket.on('Update ht value_3', (data) => {
        console.log(data);
    ...
    const getLastestDoorState = () => {
        instance.get(`/api/get_door_lastest_state?room_id=3`).then((response) => {
    ...
    const getLatestHTvalue = () => {
        instance.get(`/dht/getLatestValue?room_id=3`).then((response) => {
    ...
    const get_room_devices = () => {
        instance.get('/roll_data_room3').then((response) => {
            console.log(response.data);
                                ...
                                <View style={{ backgroundColor: '#497E3C', flex: 6, borderRadius: 10 }}>
                                    ...
                                    <View style={{ flex: 1, margin: 5, flexDirection: 'row' }}>
                                        <View style={{ backgroundColor: '#C6CDC6', flex: 1, margin: 10, justifyContent: 'center', alignItems: 'center' }}>
                                            <TouchableOpacity
                                                onPress={() => { send_light_bulb_command_on('light_bulb03') }}
                                            >
                                                <Text>ON</Text>
                                            </TouchableOpacity>
                                        </View>
                                        ...
                                        <View style={{ backgroundColor: '#868986', flex: 1, margin: 10, justifyContent: 'center', alignItems: 'center' }}>
                                            <TouchableOpacity
                                                onPress={() => { send_light_bulb_command_off('light_bulb03') }}
                                            >
                                                <Text>OFF</Text>
                                            </TouchableOpacity>
                                        </View>
